...
- **자동 업데이트**: 크론 작업이 6시간마다 새 Claude Code 버전을 확인하고 Pull Request를 자동 생성합니다. 릴리즈 직후에도 이어서 실행되므로, 밀린 버전은 한 번에 PR 하나씩 순서대로 처리됩니다. 각 PR은 병합 전에 두 플랫폼을 네이티브로 빌드해 검증하며, 병합은 항상 수동입니다.
