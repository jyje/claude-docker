...
- **Auto-update**: A cron job checks for new Claude Code versions every 6 hours and automatically creates a Pull Request. It also runs right after each release, so a backlog of versions is worked through one PR at a time. Each PR is built natively on both platforms and reports a `version-check` status, and merging is always manual.
